...
function Search({styles, getMeteos, setLocation, setPosition}) {
  ...
  const getSuggestions = async ()=>{
    ...
    try{
      setLoading(true)
      const response = await fetch(`https://nominatim.openstreetmap.org/search?q=${input}&format=json&type=city&addressdetails=1&limit=20`,{signal: controller.signal})
  ...
  return (
    <div className="mb-3">
      <form onSubmit={(e)=>{e.preventDefault();getSuggestions()}} className={` ${styles} p-[1rem] py-[0.5rem] card rounded-[50px] min-w-[100%] flex-1 items-center xs:min-w-[350px] justify-between gap-2`}>
          <label htmlFor="search" className="cursor-pointer"><FaSearch />Ama</label>
